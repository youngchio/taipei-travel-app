...
// --- 외부 라이브러리 의존성 0% 아이콘 엔진: 문법 오류 및 빌드 중단 방지 버전 ---
...
// 대표님이 검수한 총 23개의 명소 데이터 및 요청하신 고유 이미지 URL 완벽 반영
...
const App = () => {
  ...
  const Header = () => (
    <header className={`px-6 py-4 flex justify-between items-center sticky top-0 z-50 backdrop-blur-md border-b transition-colors ${isDarkMode ? 'bg-slate-950/80 border-slate-800' : 'bg-white/80 border-slate-100'}`}>
      ...
      <button onClick={() => setIsDarkMode(!isDarkMode)} className={`p-3 rounded-2xl transition-all shadow-sm ${isDarkMode ? 'bg-slate-800 text-yellow-400' : 'bg-white border border-slate-100 text-slate-400'}`}>
  ...
  const HomeTab = () => (
    <div className="pb-32 animate-fade-in">
      <div className="px-6 py-6">
        ...
        <p className="opacity-60 text-sm font-medium text-slate-600 dark:text-slate-400 italic">23개의 검증된 핫플을 한눈에.</p>
      </div>
      <div className="px-6 mb-8">
        <div className="relative h-56 rounded-[2.5rem] overflow-hidden shadow-2xl group">
          <img src="https://love.seoul.go.kr/tmda/editor/article/2025/02/19/article_202502_13_01.png" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt="Taipei Hero" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent flex flex-col justify-end p-8 text-white">
            <span className="bg-blue-600 text-[10px] font-black px-3 py-1 rounded-full w-fit mb-3 uppercase tracking-widest">Editor's Pick</span>
    ...
    return (
      <div className="pb-44 animate-fade-in">
        <div className={`px-6 py-6 sticky top-0 z-40 border-b transition-colors ${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
          <h2 className="text-2xl font-black mb-4 text-slate-900 dark:text-white font-bold">코스 메이커</h2>
        ...
        <div className="px-6 mt-8 mb-8">
          <div className="relative h-64 rounded-[3rem] overflow-hidden border-4 border-white shadow-2xl">
            ...
            <div className="absolute bottom-4 left-4 right-4 p-3 bg-white/90 backdrop-blur-md rounded-2xl shadow-lg flex items-center gap-3 text-slate-900 border border-white">
              ...
              <p className="text-[10px] font-black uppercase tracking-tight text-slate-800 italic">Real-time Map: {filtered.length} spots found</p>
            </div>
          </div>
        </div>
        <div className="px-6 flex flex-col gap-4">
          {filtered.map(c => (
            <div key={c.id} className={`p-4 rounded-3xl border flex gap-4 transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100 shadow-sm hover:shadow-md'}`}>
              <div className="relative w-24 h-24 shrink-0 overflow-hidden rounded-2xl shadow-sm">
  ...
  const NowTab = () => (
    <div className="px-6 pb-40 animate-fade-in">
      <div className="py-8"><h2 className="text-2xl font-black mb-1 text-slate-900 dark:text-white font-bold tracking-tight">타이베이 나우</h2><p className="text-xs opacity-50 font-bold uppercase tracking-widest italic text-slate-500">Live Insights</p></div>
      <a href="https://english.metro.taipei/" target="_blank" rel="noreferrer" className="block bg-red-50 border border-red-100 p-5 rounded-[2.5rem] mb-6 flex items-start gap-4 hover:bg-red-100 transition-colors shadow-sm">
        <div className="mt-1"><Icon name="alert" size={24} color="#ef4444" /></div>
        <div className="flex-1">
          <h4 className="text-xs font-black text-red-900 mb-1">긴급: MRT 단수이선 지연 안내</h4>
          <p className="text-[10px] text-red-700 opacity-80 leading-relaxed font-bold">신호 장애로 인해 10분 지연 운행 중입니다. 공식 홈페이지에서 상황을 확인하세요.</p>
        </div>
  ...
  const LocalTab = () => (
    <div className="px-6 pb-40 animate-fade-in">
      <div className="py-8"><h2 className="text-2xl font-black mb-1 text-slate-900 dark:text-white font-bold tracking-tight">로컬 인사이트</h2><p className="text-xs opacity-50 font-bold uppercase tracking-widest italic text-slate-500">Exclusive Guide</p></div>
      <div className="bg-blue-600 p-8 rounded-[3rem] text-white mb-8 relative overflow-hidden shadow-2xl shadow-blue-100">
        ...
        <h3 className="text-2xl font-black mb-3 leading-tight tracking-tight text-shadow font-bold">중국어 한 마디로<br/>현지인 대우받기</h3>
        <p className="text-sm opacity-90 mb-6 font-medium leading-relaxed font-bold">인크로스 대표님의 현지 경험이 담긴<br/>상황별 주문 카드 & 꿀팁 가이드</p>
        <button className="bg-white text-blue-600 px-6 py-3 rounded-2xl text-xs font-black shadow-xl hover:scale-105 active:scale-95 transition-all font-bold">카드 확인하기</button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        {[
          ...
          <div key={idx} className={`p-6 rounded-[2rem] border transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100 shadow-sm hover:border-blue-400 hover:shadow-lg'}`}>
            <span className="text-[9px] font-black text-blue-500 mb-2 block uppercase font-bold tracking-widest">{item.tag}</span>
    ...
    return (
      <div className="px-6 pb-44 animate-fade-in">
        <div className="flex justify-between items-end py-8 text-slate-900 dark:text-white">
          <h2 className="text-2xl font-black font-bold tracking-tight">나의 예산 리포트</h2>
          <div className="flex items-center gap-3">
            <span className="text-xs font-bold opacity-50 italic text-slate-600 dark:text-slate-400 font-bold">{myCourses.length} spots</span>
            {myCourses.length > 0 && <button onClick={clearAll} className="text-[10px] font-black text-red-500 flex items-center gap-1 bg-red-50 px-3 py-1.5 rounded-xl transition-colors hover:bg-red-100 font-bold"><Icon name="trash" size={12}/> 비우기</button>}
          </div>
        </div>
        {myCourses.length === 0 ? (
          <div className={`flex flex-col items-center justify-center py-24 rounded-[3.5rem] border-2 border-dashed ${isDarkMode ? 'border-slate-800 bg-slate-900/50' : 'border-slate-100 bg-slate-50'}`}>
            ...
            <p className="text-sm font-black opacity-30 text-slate-500 font-bold">담긴 코스가 없습니다.</p>
            <button onClick={() => setActiveTab('home')} className="mt-4 text-blue-600 text-xs font-black underline underline-offset-4 font-bold uppercase tracking-widest">코스 담으러 가기</button>
          </div>
        ) : (
          <>
            <div className={`p-6 mb-8 rounded-[3rem] border ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100 shadow-xl shadow-blue-50'}`}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h4 className="text-xs font-black uppercase tracking-widest opacity-60 text-slate-500 font-bold">Expenditure Analysis</h4>
              </div>
              <div className="flex h-4 rounded-full overflow-hidden mb-4 bg-slate-100 dark:bg-slate-800 shadow-inner">
                <div style={{ width: `${(foodSum/totalTwd)*100}%` }} className="bg-yellow-400 transition-all duration-700 shadow-sm"></div>
                <div style={{ width: `${(tourSum/totalTwd)*100}%` }} className="bg-blue-500 transition-all duration-700 shadow-sm"></div>
              </div>
              <div className="flex justify-between text-[11px] font-black text-slate-800 dark:text-slate-200">
                <div className="flex items-center gap-2 font-bold"><div className="w-2.5 h-2.5 rounded-full bg-yellow-400 shadow-sm"/><span>식비: {foodSum.toLocaleString()} TWD</span></div>
                <div className="flex items-center gap-2 font-bold"><div className="w-2.5 h-2.5 rounded-full bg-blue-500 shadow-sm"/><span>기타: {tourSum.toLocaleString()} TWD</span></div>
              </div>
            </div>
            <div className="flex flex-col gap-4 mb-8">
              {myCourses.map(c => (
                <div key={c.uniqueId} className={`flex items-center gap-4 p-4 rounded-3xl transition-all ${isDarkMode ? 'bg-slate-900 border border-slate-800' : 'bg-white shadow-sm border border-slate-50'}`}>
                  ...
                  <div className="flex-1">
                    <h4 className="text-xs font-black mb-1 text-slate-800 dark:text-slate-200 font-bold">{c.title}</h4>
            ...
            <button className="w-full flex justify-center items-center gap-3 bg-yellow-400 text-yellow-950 font-black py-5 rounded-[2rem] shadow-xl shadow-yellow-100 hover:bg-yellow-300 active:scale-95 transition-all mb-24 font-bold text-lg">
              <Icon name="message" size={20} fill="currentColor" /> 카카오톡으로 일정 공유하기
            </button>
          </>
        )}
        <div className={`fixed bottom-28 left-6 right-6 p-7 rounded-[2.5rem] shadow-2xl transition-all duration-500 flex justify-between items-center z-50 transform border border-white/10 ${myCourses.length > 0 ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-20 opacity-0 scale-95 pointer-events-none'} ${isDarkMode ? 'bg-slate-900 text-slate-100 shadow-blue-900/20' : 'bg-slate-900 text-white shadow-slate-900/40'}`}>
          <div className="text-slate-100">
            <p className="text-[9px] font-black opacity-40 uppercase tracking-[0.2em] mb-1 font-bold">Estimated Total</p>
            <div className="flex items-baseline gap-1">
              <span className="text-3xl font-black tracking-tighter font-bold">{totalTwd.toLocaleString()}</span>
              <span className="text-[10px] font-bold opacity-40 uppercase font-bold">TWD</span>
            </div>
          </div>
          <div className="text-right">
            <p className="text-xl font-black text-blue-400 tracking-tighter shadow-sm font-bold italic">≈ {totalKrw.toLocaleString()}원</p>
            <p className="text-[8px] font-black opacity-30 mt-1 uppercase tracking-widest italic font-bold">Rate 1:42.5 applied</p>
          </div>
        </div>
      </div>
    );
        ...
        <nav className={`fixed bottom-0 left-0 right-0 max-w-md mx-auto px-8 pt-4 pb-12 flex justify-between items-center z-50 backdrop-blur-xl border-t ${isDarkMode ? 'bg-slate-950/90 border-slate-800' : 'bg-white/90 border-slate-100 shadow-[0_-4px_30px_rgba(0,0,0,0.04)]'}`}>
          {[
            ...
            <button key={tab.id} onClick={() => { setActiveTab(tab.id); window.scrollTo(0,0); }} className={`flex flex-col items-center gap-1.5 transition-all relative ${activeTab === tab.id ? 'text-blue-600 scale-110' : 'text-slate-400 opacity-40 hover:opacity-100'}`}>
              ...
              <span className="text-[10px] font-black font-bold uppercase">{tab.name}</span>
              {tab.id === 'mytrip' && myCourses.length > 0 && (
                <span className="absolute top-1 right-1 w-4 h-4 bg-red-500 rounded-full border-2 border-white dark:border-slate-950 flex items-center justify-center text-[8px] font-black text-white shadow-sm animate-pulse font-bold">
                  {myCourses.length}
                </span>
              )}
            </button>
